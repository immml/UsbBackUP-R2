...
插入 U 盘
   ...
   ├─► 不是可移动卷？──► 跳过
   │
   ├─► 盘根有 .usbbackup-allow ？──► 有 → **本卷豁免**，什么都不做
   │                                  （自己的工具盘走这条，防止私钥被传上网）
   │
   ├─► 同物理磁盘上任一卷根有 .usbbackup-allow-disk ？──► 有 → **整支盘豁免**，什么都不做
   │                                  （多分区盘必须靠它，否则其余分区会被采集）
   │
   ├─► 采集策略
   │     all          → 放行
...
2. （可选，**本项目不启用**）给桶加 `Bucket lock rules`（桶锁）——prefix 填 `usb/`。理由与代价见 §1.1。
...
所以本项目的配套手段是 **定期轮换 token**——换一份 `client.json` 就完事，不必重新编译客户端。

> **2026-09-20 决定：不启用桶锁。** R2 上确实有一条能护住"可用性"的原生机制（Bucket locks），本项目**明确不启用**，改为接受"持凭据者可删除远端对象"的残余风险。理由之一是它连你自己也一起挡：锁规则配上去之后**你自己也删不掉被锁的对象**，且配了规则的桶**不能被清空**——系统还在校准阶段，这会连测试残留都清不掉。

别指望"凭据就算被拿到也没关系"：泄漏的凭据能读、能覆盖、能删除。加密只保护**内容**（没有私钥读不出明文），保护不了**可用性**——本项目对"可用性"这一半**不做兜底**，只靠定期轮换凭据压缩暴露窗口。

> **R2 没有对象版本控制。** 网上不少文章说它"完整支持 S3 的版本控制 API"，实际不是：`PutBucketVersioning` / `GetBucketVersioning` 都在 R2 的 S3 兼容表里标为**未实现**，而那个能返回 200 的 `GetBucketVersioning` 是 2022-07-20 加的 **dummy 接口**，永远返回"从未开启"，读不出真实状态（`ListObjectVersions` 则直接 501）。所以防删只有**桶锁**这一条路——而本项目已决定不启用它。
>
> （留档，本项目未启用）控制台路径：**R2 → 选桶 → Settings → Bucket lock rules → Add rule**（prefix 填 `usb/`，保留期可选 indefinite）。桶锁**优先于 lifecycle**，且配了锁规则的桶**不能被清空**。用 API 配置需要 **Cloudflare API token（Bearer）**，不是 R2 的 S3 凭据。
...
- 盘根有没有授权标记（卷级有没有被豁免）
- 同一物理磁盘上有没有磁盘级标记（整支盘有没有被豁免）——查磁盘号需要管理员权限，普通权限下这里会写"未能检查：…"，那不是"没有标记"
- 采集策略会不会放行
- 容量有没有超阈值
- 最终会走"打包 + 加密 + 上传"还是"跳过"
- 上传通道是否可用（凭据能否在本机解开）
...
```
I:\
├── .usbbackup-allow                 ← 授权标记（卷级），必须留在盘根
├── .usbbackup-allow-disk            ← 授权标记（磁盘级），必须留在盘根
└── backup\tools\
...
盘根还有一个 `.usbbackup-allow-disk`，作用是把豁免范围从**这个分区**放大到**整块物理磁盘**。为什么必需：一支 Ventoy 盘有两个分区（数据区 + VTOYEFI），只在数据区放标记时，客户端扫到固件区会认定"没标记"，照样整盘打包上传——这是实测踩到过的：服务一启动就把 `J:`（27.44 MiB）整盘采集并开始上传。有了磁盘级标记，只要**同一物理磁盘上任意卷根**带着它，该盘所有分区一起放过。判定时会去查物理磁盘号（需要管理员权限；客户端以服务运行时天然满足），普通权限下这一层会失效并**明确告警**，不会静默当成"没有标记"。

**别把两个标记挪进子目录。** 豁免判定只在 `<盘根>\` 处 `Stat`；挪走就检测不到，这个带着私钥的盘就会被当成普通介质打包上传。

**卷级那个文件名与项目 A（不含出站能力的版本）共用。** 装盘时只**追加**指纹、绝不覆盖：A 的判定是逐行比对指纹的，盘根可能已经有 A 写的那行；覆盖会让 A 认不出这块盘，于是 A 把它当普通介质扫描打包——而这块盘里有私钥。所以 `install-usb` 是追加 + 幂等的，重复装盘不会覆盖也不会失败，装完输出里会写明走了哪条分支（新建 / 追加 / 已含相同指纹）。磁盘级那个是本项目新增的，A 不认它——但 A 只看卷级那个，所以不受影响。
...
| 作业直接跳过、日志写 `exempt-marker` | 本卷盘根有 `.usbbackup-allow`，被豁免了。这是期望行为 |
| 作业直接跳过、日志写 `exempt-disk-marker` | 同物理磁盘上另一个分区有 `.usbbackup-allow-disk`，整支盘被豁免。日志里会写明命中的是哪个卷根。同样是期望行为 |
| 日志 WARN「磁盘级授权标记本次未生效」 | 查不到本卷的物理磁盘号（普通权限下打不开卷句柄）。此时只做了卷级判定——用管理员权限跑，或装成服务 |
...
- [ ] R2 token 是 `Object Read & Write` + **仅限目标桶**，不是 Admin 档，且有轮换计划
- [ ] 已知情：R2 无对象版本控制、本项目不启用桶锁 ⇒ **远端对象没有防删兜底**（README §5.1.1）
...
- [ ] 工具盘根上**两个**授权标记都在：`.usbbackup-allow` 与 `.usbbackup-allow-disk`（多分区盘少了后者，其余分区会被采集）
