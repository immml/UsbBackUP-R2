...
// 「Object Read & Write + 仅限目标桶」，配套定期轮换（R2 无对象版本控制）。
func cmdCred(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	fmt.Fprintln(stdout, "     因此这一档同时能读/覆盖/删除，且 R2 没有对象版本控制，删除**没有原生兜底**；")
	fmt.Fprintln(stdout, "     请**定期轮换** token。要真正做到限前缀只能用临时凭据（TTL ≤ 7 天），不适合常驻客户端。")
