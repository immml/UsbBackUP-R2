...
| 授权标记（卷级） | `.usbbackup-allow`（两边同名，工具盘互相认） |
| 授权标记（磁盘级） | `.usbbackup-allow-disk`（本项目独有；A 不认它，但也不受影响，见 §5.5） |
...
所以**两者可以装在同一台机器上**：服务名、配置目录、环境变量、密钥文件名全部独立，谁都不会覆盖谁。**唯一故意同名的是卷级授权标记**——两个项目的工具盘都要被对方认出来是"自己人的盘"，名字不同反而会互相采集。本项目在这一层之上**另写了一个磁盘级标记**（`.usbbackup-allow-disk`，见 §5.5）：A 不认它，但 A 只看卷级那个，所以不受影响。
...
卷到达事件
  ...
  │
  ├─ 盘根有 .usbbackup-allow ？ ── 有 → **本卷豁免**，直接结束
  │
  ├─ 同物理磁盘上任意卷根有 .usbbackup-allow-disk ？ ── 有 → **整支盘豁免**，直接结束
...
| `config-check` | 校验配置与运行环境（会创建产物目录以验证可写） |
...
```
I:\
├── .usbbackup-allow        ← 授权标记（卷级），必须留在**盘根**
├── .usbbackup-allow-disk   ← 授权标记（磁盘级），同样必须在**盘根**
└── backup\tools\
...
**为什么必须有两个标记（`.usbbackup-allow-disk` 的存在理由）**：卷级标记只认"写了它的那个卷根"，而人的直觉是"这支 U 盘"。多分区介质上两者必然分叉——**一支 Ventoy 盘有两个分区**（数据区 `I:` + VTOYEFI `J:`），只在数据区放标记时，客户端扫到固件区会认定"没标记"，照样整盘打包上传。这不是推演，是 2026-09-20 部署实测踩到的：服务一启动就把 `J:`（27.44 MiB）整盘采集并开始上传。磁盘级标记把豁免范围提到**整块物理磁盘**：只要同盘**任意**卷根带着它，该盘所有分区一起放过。

判定需要查物理磁盘号（`IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS`），这一步要管理员权限。客户端以服务运行时是 LocalSystem，权限充足；手工用普通权限跑 `once` 时这一层会失效——此时会**明确告警**（`once` 输出里显示"未能检查：…"、日志一条 WARN），绝不静默当成"没有标记"。

**卷级那个文件与项目 A（不含出站能力的版本）共用，装盘时只追加、绝不覆盖。** 原因是 A 的判定要**逐行比对指纹**：盘根可能已经有 A 写的 `fingerprint=…`，整体覆盖会让 A 认不出这块盘，于是 A 把它当普通介质扫描打包——而这块盘里有私钥。一次"顺手覆盖"就足够造成私钥外泄，所以 `install-usb` 是追加 + 幂等的（重复装盘不会越写越长）。

### 4.6.1 这里为什么不做"凭据硬编码进客户端"（决策 D-16 复核）

需求方提过：把 R2 凭据加密后硬编码进 `client.exe`，省掉部署时的一切凭据步骤。**做不到，而且做出来是负收益。**

客户端是纯 Go 静态 exe，**没有可执行文件之外的密钥来源**。任何加密方案都得把解密密钥一起放进同一份 exe——`strings` 或一次内存转储就能同时拿到密钥与密文。真想抬高逆向成本只能上 garble 之类的混淆器，而那会破坏本项目"仅标准库、可离线构建"的硬约束（从项目 A 一路守到现在，是明确取舍）。换来的只是"把提取成本从几分钟抬到几小时"，**不改变可提取性**。

更关键的是可运维性，这一条比"能不能被提取"要紧：

| | 硬编码进 exe | 独立 `client.json` |
|---|---|---|
| 轮换 token | **重新编译 + 重新分发全部目标机的客户端** | 换一个文件 |
| 权限收窄 | 同上 | 同上 |
| 现场排查 | 要去二进制里找 | `type client.json` 就能看端点/桶/前缀 |
| 泄露后 | 每台机器都要换 | 换文件 + 吊销 token |

所以**「不想做口令」这个诉求不靠硬编码解**，靠下面 §4.6.2：把凭据在**目标机上**生成成 DPAPI 机器绑定档，之后这台机器再也不用提供任何口令。代价是重装系统要重生成一份。

> 想真正"客户端自带凭据"只有一条路：**把凭据内嵌进 exe 的行为本身**，即 `usbkeygen-r2` 在**目标机上**现跑一次——那和 §4.6.2 是同一件事，只是没有把它藏进二进制而已。

### 4.6.2 不设环境变量的做法：在目标机上现生成一份 DPAPI 凭据（**推荐**）

盘上带的是哪一份凭据，决定了部署方式完全不同：

| 凭据的保护方式 | 盘插到别的 Windows 机器上 |
|---|---|
| `dpapi-machine`（机器绑定） | **能用，且不需要任何口令/环境变量**——但必须是在**这台**机器上生成的 |
| `passphrase-aesgcm-v1`（口令加密） | 能直接用（机器无关），但要给客户端口令 → 要设环境变量 |
| `plain-file-0600` | 能直接用，但等于把 R2 访问权明文放在盘上 |

**首选做法是让盘上**不**带凭据，到目标机上现生成一份机器绑定的。** 盘上那份只作为"当场重生成"的素材（`r2.json`，secret 留空）：

```CMD
REM 目标机器 · CMD（先 cd 到工具所在的本地目录，例如 C:\backup-agent）
REM 1) 把工具从盘上拷到本地目录（服务要绑本地路径，不能绑盘符，见 §4.6.3）

REM 2) 现生成一份 DPAPI 凭据：--from 提供路由信息，Secret 交互输入（无回显，不落盘）
usbkeygen-r2.exe cred --from r2.json --out client.json --scope both --force
REM r2.json 里把 secret_access_key 留空，运行时才会问；清了就不会留在文件里

REM 3) 验证：一个环境变量都不设也应该能列出生效凭据
client.exe cred-check
```

`cred` 不带 `--cred-pass-file` / `--cred-pass` / `--plain-file` 时**默认就是 `dpapi-machine`**（Windows 上）。它走 `CRYPTPROTECT_LOCAL_MACHINE`，即**机器范围而非用户范围**——同机上**任何账户**都能解开，包括以 LocalSystem 运行的客户端服务。这不是疏漏，是刻意选的：生成凭据的是交互登录的管理员，跑服务的是 LocalSystem，用户范围在两者之间解不开。已实测：不设任何环境变量，`client.exe cred-check` 正常列出凭据。

> **两个坑必须先知道**：
>
> 1. 客户端找凭据的顺序是 **① `client.exe` 同级 → ② 当前工作目录 → ③ `%LOCALAPPDATA%\usbbackup-r2\`**。所以只要**工具盘**上那份 `client.json` 还躺在 `client.exe` 旁边，它就会**盖住**你在本地目录生成的那份——现场看着像"DPAPI 不生效"，其实是压根没被读到。
> 2. 反过来，**不要往工具盘上生成** DPAPI 凭据——那会把盘上那份机器无关的通用凭据换成只属于这一台机器的，盘就废了。
>
> 由此一条硬约定：**把工具拷到目标机的本地目录**（如 `C:\backup-agent`）再生成凭据、装服务。这一下同时解决三件事：服务不绑 U 盘盘符、凭据可以走 DPAPI、环境变量真的不必设了。

**代价**：凭据与机器绑定，**重装系统或换机就要重生成**；口令加密那份则是机器无关的。
选哪条，看你的目标机是"装好就不动"还是"经常重装"。

### 4.6.3 服务必须装在本地路径，不能装在盘上

`install-service` 注册的 `binPath` 取的是 `os.Executable()`，也就是**你执行它的那个 exe 的绝对路径**。所以直接在 `I:\backup\tool-R3\` 里跑 `client.exe install-service`：

- 服务**绑死 U 盘盘符**，拔盘就起不来；
- 凭据连带解析到盘上那份（§4.6.2 的坑 1），DPAPI 那份永远读不到；
- 多台机器共用一块盘时更走不通。

正确形态：**先把工具拷到目标机本地目录，在那里生成凭据 + 装服务。**

---

#### 备用方案：盘上带口令加密凭据 + 设环境变量

只有在"目标机经常重装、又不想每次重生成凭据"时才选这条。要点是——**漏掉口令不会报错，但上传会被跳过、产物只留在本地**，现场表现为"跑了半天，R2 上什么都没有"：

```PowerShell
# PowerShell（目标机器上，注册服务前）
# 1) 口令单独存一个文件（别放工具盘、别和客户端同目录——同目录会让强度退回明文）
...
所以本项目的落地形态是 **`Object Read & Write` + 仅限目标桶**，配套的运维手段是 **定期轮换 token**——换一份 `client.json` 就完事，不需要重新编译客户端。

> **2026-09-20 决定：不启用桶锁。** R2 上另有能兜住"删除"的原生机制（Bucket locks），本项目**明确不启用**，改为接受"持有上传凭据者可以删除远端对象"这一残余风险。因此 **§5.1.1 降级为备查资料**，不再是部署前提，检查清单里也不再要求。
...
- **删除**——**没有原生兜底**：R2 无对象版本控制，桶锁本项目不启用（见下）。这一半只能**接受**，靠定期轮换凭据压缩暴露窗口。

#### 5.1.1 桶锁（Bucket locks）——备查：R2 没有版本控制，这是唯一能兜住删除的机制

> **本项目不启用这一节所述的机制**（2026-09-20 决定），此处仅作技术备查与决策留痕。
> 客观影响供参考：锁规则一旦配上，**你自己也删不掉被锁住的对象**，且配了规则的桶**不能被"清空"**——对仍在校准阶段的系统，这会连测试残留都清不掉。
...
**为什么它技术上对症**：上传凭据是 `Object Read & Write`，**能删对象、但不能改桶配置**——也就是**动不了锁规则**。于是"凭据泄漏后被恶意删库"这条被堵死，而你自己走控制台仍留了后门。这正是版本控制本来要提供的那层保护。**（本项目权衡后仍选择不启用，见本节开头。）**
...
> **保留期怎么选（仅在决定启用时适用）**：`Indefinite` = 备份永不删，且必须先到控制台删规则才能清库——这才是真正的 WORM 语义；填固定天数（如 365）则到期后恢复可删，便于定期清理旧备份。
>
> **一个已作废的待实测点**：锁规则是否影响 `AbortMultipartUpload`（上传失败时我们主动中止分片，不清会留下**计费**的残留分片）。官方文档只写了"桶不能被清空"，没写分片这一侧。**因为决定不启用桶锁，这个实测点不再需要。** 已实测的是**未加锁状态下**的行为（2026-09-20，`r2ver.py locktest`）：`_probe/` 前缀 Put 200 → Delete 204；`usb/` 前缀 CreateMultipart 200 → UploadPart 200 → **Abort 204**；`ListMultipartUploads` 残留 **0**。⇒ 本项目依赖的失败回滚路径在现状下完好。
...
> 对象键带 UTC 时间戳，所以**每次上传都是新键**。这防的是"覆盖"，不防"删除"——一旦有凭据泄漏，历史对象是可以被删掉的。而 R2 **没有版本控制**可救，桶锁本项目又不启用（见 §5.1.1），**这一半没有兜底**，只能靠定期轮换凭据压缩暴露窗口。
...
1. 这份 token 就是 R2 可达的最小档位（`Object Read & Write` + 仅限 `usbbackup` 桶）。它**能读、能覆盖、能删除**——这不是配置失误，是 R2 没有"只写不读"档位。所以**删除这一半没有兜底**（R2 无对象版本控制；桶锁本项目不启用，§5.1.1），只能靠定期轮换凭据压缩暴露窗口。
...
1. **关于版本控制，前一次结论作废。** 当时由 `200` + 空 `<VersioningConfiguration/>` 推断"该桶未开启版本控制"——**这个推断不成立**。R2 的 `GetBucketVersioning` 是 2022-07-20 加的 **dummy implementation**，按发布说明它永远"mimic the response that a basic AWS S3 bucket will return when first created"，即**不管实际情况怎样都返回空**；而 `PutBucketVersioning` 在 R2 的 S3 兼容表里根本没实现。**事实是：R2 不提供对象版本控制，"开没开"这个提法本身就没有对象。** 备查：R2 上另有能兜住删除的**桶锁**（§5.1.1），本项目已决定不启用。
...
> **路径字段写的是模板，不是绝对路径——这是刻意的。** 生成器会把整份配置序列化后内嵌进 `client.exe`，而客户端常常在另一台机器上部署；若把构建机的绝对路径烙进去，目标机上的日志、审计、产物、凭据就全指向一个不存在的用户目录（详见 §8.3 第 20 条）。`%LOCALAPPDATA%` / `%TEMP%` 在**使用它的那台机器上**展开；`ExpandPath` 对变量缺失或为空也有兜底。只有你显式用 `--output-dir` 指定绝对路径时才会定死。
...
```json
{
  ...
  "collect": {
    ...
    "exempt_disk_marker_file": ".usbbackup-allow-disk"
  },
...
### 5.5 三个标记：两个管豁免，一个管采集

| 标记 | 位置 | 含义 | 有无它 |
|---|---|---|---|
| `.usbbackup-allow` | **盘根**（只认盘根） | "这个卷是我自己的，别动它" | 有 → **该卷豁免**，任何策略档位下都不采集 |
| `.usbbackup-allow-disk` | **盘根**（同盘任一卷即可） | "这支盘是我自己的，别动它" | 有 → **同一物理磁盘上所有卷一起豁免** |
| `.usbbackup-collect` | **盘根** | "这个卷的内容可以打包上传" | 仅 `marker_only` 档位使用 |

两级授权标记的判定**都不受策略档位影响**，任何档位都先做这两步。原因很实际：工具盘上放着私钥，一旦被采集上传就等于把私钥发布到网上——这是本项目最不能出的事故，不允许"因为策略是 all 所以跳过豁免检查"这种组合存在。

**为什么要有两级**：`.usbbackup-allow` 的作用域是**卷**，而那与使用者的心智模型（"这支盘"）在多分区介质上不一致——Ventoy 盘的数据区放了标记，固件分区照旧被采集（README §4.4 有完整经过）。`.usbbackup-allow-disk` 补的就是这一层。只放卷级那份时，行为与以前完全一样；多放一个文件就豁免整支盘。

> `.usbbackup-allow` 这个文件名与项目 A 共用，**语义不能改**（A 也按卷级判定）。所以"整支盘"这件事用了一个新文件名来表达，而不是给它加戏。

### 5.6 环境变量

`USBBACKUP_R2_*` 系列可覆盖同名配置项（共 16 个）：
...
$env:USBBACKUP_R2_COLLECT_EXEMPT_DISK_MARKER = '.usbbackup-allow-disk'
...
- [ ] R2 token 用的是 `Object Read & Write` + 仅限目标桶，**不是** Admin 档，且有轮换计划
- [ ] 已知情：R2 无对象版本控制、本项目不启用桶锁 ⇒ **远端对象没有防删兜底**（§5.1.1）
- [ ] `r2-check` 通过（含权限范围检查）
- [ ] `probe --drive <盘符>` 的判定结论符合预期
- [ ] 私钥已离线备份，且**不在**客户端分发的目录里
- [ ] 工具盘根上**两个**授权标记都在：`.usbbackup-allow` 与 `.usbbackup-allow-disk`（多分区盘少了后者，其余分区会被采集，§4.4）
- [ ] 目标机上的客户端能查到物理磁盘号（服务身份天然可以；普通权限跑 `once` 会告警"未能检查"，见 §5.5）
- [ ] 目标机上**工具已拷到本地目录**（不是直接在盘上跑，见 §4.6.3）
- [ ] 凭据已确认能在目标机解开：`client.exe cred-check` 通过（DPAPI 档不需要环境变量）
...
│   ├── collectpolicy/    采集准入判定（卷级/磁盘级豁免 + 三档策略）
...
测试：**18 个包有测试，共 256 个用例**。核心路径全在上——容量门控、卷标净化、路径守卫与 Zip Slip、加解密往返、篡改/截断/错误密钥拒绝、SigV4 官方已知答案向量、假 S3 端到端（含分片与 Abort）、采集策略与两级豁免（卷级 / 磁盘级 / 查询失败告警 / 同名冲突校验）、上传编排与本地去留矩阵、内嵌配置往返与私钥守卫、**内嵌配置里不得出现构建机绝对路径**、默认路径必须是可展开模板、凭据三种保护方式（DPAPI / 口令 / 明文）的往返与拒绝路径、配置文件的按键合并、`--from` 与命令行开关的优先级、上传进度节流的去重、装盘时的门控注入与两个授权标记的追加合并、上传开关的注入与"素材不得带 secret"的拒绝路径、盘内 README 按实际开关状态分派的措辞。
...
单测没有发现、实际跑起来才暴露的问题（这类问题在本项目里出现过 20 次）：
...
18. **授权标记只豁免"写了它的那个卷根"，而人的直觉是"这支 U 盘"。** 单分区介质上这两件事重合，多分区介质上必然分叉——而本项目自己的工具盘就是多分区的：数据区 `I:` 放了标记，固件区 `J:`（VTOYEFI，27.44 MiB）没有，于是 Windows 服务一启动就把 `J:` 整盘采集并开始上传。判定本身没写错（就是 `Stat <卷根>\.usbbackup-allow`），错在**语义粒度选在了"卷"这一层**。修法是新增一个**磁盘级**标记 `.usbbackup-allow-disk`：同一物理磁盘上任意卷根带着它，整支盘一起豁免（查磁盘号走 `IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS`；这一步需要管理员权限，权限不足时**明确告警**、绝不静默降级）。**没有去改 `.usbbackup-allow` 的语义**——那个名字与项目 A 共用，改它等于让两边的判定悄悄分叉；要表达"整支盘"就再加一个文件，显式可见。教训：任何涉及"作用域"的判定都要问一句"这个作用域在真实介质上会不会被拆开"，别拿单分区样本当全部。

19. 刚做完磁盘级豁免、拿 `subst` 造的虚拟卷跑端到端时，`CreateFileW(\\.\V:)` 报 `Access is denied`，而错误文案当时写的是"**通常需要管理员权限**"——可那个会话**本来就是管理员**（同一次运行的 `C:` / `I:` / `J:` 都查得到磁盘号）。真实成因是这个卷根本没有对应的设备对象，跟权限无关；照着提示去提权只会白跑一趟。修法是把文案改成并列两种可能、**不当断言原因**。根因与 §8.3 第 14 条相同：**错误信息里不要写"最可能的原因"，除非能验证它。**

20. **生成器把构建机的绝对路径烙进了 client.exe。** `config.Default()` 里的产物目录、公钥路径、审计文件、日志文件、凭据路径，原本都是**构建时解析好的绝对路径**（`C:\Users\<构建者>\AppData\Local\...`）；而整份配置会被序列化后内嵌进客户端，客户端却常常在**另一台机器**上部署。于是目标机上的日志、审计、产物、凭据全都指向一个不存在的用户目录，系统里还会凭空多出一个别人名字的 profile 树。换台机器之所以不报错，原因很隐蔽：`logx` 在日志目录建不出来时会**降级**为仅控制台，而服务模式 `useConsole=false`，于是落到 `io.Discard`——**服务照常跑，一行日志都不落**，那句降级警告写在 stderr 上，对没有控制台的服务同样是黑洞。发现方式是本机完整部署之后，**换一个 `%LOCALAPPDATA%` / `%TEMP%` 再跑 `config-check`**：输出纹丝不动，说明这些值根本没有在运行时解析。修法是把默认值改成 `%LOCALAPPDATA%` / `%TEMP%` 模板（所有消费点本来就走 `ExpandPath`），并补上变量缺失或为空时的兜底。顺带一提，README 的示例配置里**本来就写着** `%TEMP%\\backup`——所以这同时是一次"文档说对了、代码没做到"的修正。回归测试钉两条：`Default()` 的路径不得带盘符且必须可展开；**clientgen 产出的内嵌配置里不得出现构建机的 `LOCALAPPDATA`**。
