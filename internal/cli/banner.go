...
================================================================================
  ...
  [1] 使用范围
      ...
      客户端按【显式采集策略】处理介质：
        · 盘根有 .usbbackup-allow  → **本卷**【豁免】，不读取、不打包、不上传；
        · 同一物理磁盘上任意卷根有 .usbbackup-allow-disk → **整支盘**【豁免】，
          含盘上其它分区（多分区介质如 Ventoy 盘必须靠它，否则其余分区会被采集）；
...
const NoticeShort = `[安全提示] 本工具仅限用于本人自有的计算机与存储介质。
  ...
  · 采集范围由显式策略控制；盘根有 .usbbackup-allow 的卷、以及同物理磁盘上
    带 .usbbackup-allow-disk 的整支盘，一律豁免（不采集）；
