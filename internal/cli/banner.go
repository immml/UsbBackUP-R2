...
//
// 文案要点（G-01 / G-04 / G-06′ / G-07 / G-12 / G-15）：
//   - 仅限本人自有的机器与介质；
//   - 不读取、不复制、不外传任何私钥内容；
//   - 不删除、不改写源介质上的任何文件；
//   - 唯一出站是向你自己配置的 R2 端点上传加密产物，无入站、无远程指令；
//   - 无隐蔽能力、无免杀处理；
//   - 私钥一旦丢失，密文不可恢复。
const WarningBanner = `================================================================================
...
================================================================================
  ...
  [1] 使用范围
      ...
      客户端按【显式采集策略】处理介质。默认档位下，未带私钥的介质一律采集
      （受容量上限约束）；另有 marker_only（仅带标记的介质）与 off（关闭采集分支）。
  ...
  [3] 关于数据写入与上传
      ...
      · 未授权分支：只【读取】该介质内容用于打包加密，源文件一律保留。
        绝不删除源介质上的任何文件。
      · 未授权分支加密完成后，容器会被【上传到你配置的 Cloudflare R2 桶】。
  ...
  [5] 关于网络与隐蔽性
      ...
      网络方面：唯一的出站是向【你自己配置的 R2 端点】上传加密容器，仅 HTTPS
      且强制校验证书。没有入站监听、不接受任何远程指令、不存在控制通道，
      也不做遥测上报；除该端点外不会访问任何地址。

      客户端持有的 R2 凭据存放于本机受保护文件（DPAPI 加密，仅限本机管理员
      或 SYSTEM 读取），应授予最小权限并定期轮换。任何落在机器上的凭据都存在
      被本机管理员提取的可能——本工具【不声称】它不可提取。
...
const NoticeShort = `[安全提示] 本工具仅限用于本人自有的计算机与存储介质。
  ...
  · 采集范围由显式策略控制（默认采集未带私钥的介质，受容量上限约束）；
  · 未授权分支的加密容器会上传到你配置的 R2 桶（唯一出站，仅 HTTPS）；
